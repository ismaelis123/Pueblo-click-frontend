...
import { FiCheck, FiClock, FiPackage, FiPhone, FiUser, FiTruck } from 'react-icons/fi';
...
const MyOrders = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 pb-20 md:pb-8">
      <h1 className="text-2xl font-bold text-text mb-6">Mis Órdenes Asignadas</h1>
              ...
              <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                ...
                {order.status === 'delivered' && (
                  <div className="flex items-center gap-1 text-yellow-600">
                    <FiClock /> Esperando confirmación del cliente
                  </div>
                )}
                
                {order.status === 'completed' && (
                  <div className="flex items-center gap-1 text-secondary">
                    <FiCheck /> Completada - C${order.amount} ganados
                  </div>
                )}
              </div>
            </div>
