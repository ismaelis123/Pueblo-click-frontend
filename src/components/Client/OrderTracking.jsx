...
import { FiArrowLeft, FiMapPin, FiNavigation, FiClock, FiCompass, FiMap, FiUser, FiTarget } from 'react-icons/fi';
...
const OrderTracking = () => {
  ...
  useEffect(() => {
    if (L && mapContainerRef.current && (order?.pickupLocation || order?.deliveryLocation || mandaditoLocation)) {
      initMap();
    }
  ...
  const initMap = () => {
    if (!mapRef.current && mapContainerRef.current && L) {
      ...
      else if (order?.pickupLocation?.lat) center = [order.pickupLocation.lat, order.pickupLocation.lng];
    ...
    <Background>
      <div className="container mx-auto py-4 px-4 pb-24 md:pb-8">
        <div className="flex items-center gap-3 mb-4">
          <button onClick={() => navigate(-1)} className="p-2 hover:bg-gray-100 rounded-full"><FiArrowLeft className="text-xl" /></button>
          <h1 className="text-xl font-bold text-gray-800">Seguimiento del Pedido</h1>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-4">
          <div className="flex justify-between items-center mb-3">
            <div><p className="text-sm text-gray-500">Orden #{order?._id?.slice(-6)}</p><p className="font-semibold text-gray-800 flex items-center gap-2"><FiUser className="text-[#FF6B35]" /> {order?.mandadito?.name || 'Mandadito asignado'}</p></div>
            <div className={`px-3 py-1 rounded-full text-sm font-medium ${order?.status === 'accepted' ? 'bg-blue-100 text-blue-700' : order?.status === 'delivered' ? 'bg-yellow-100 text-yellow-700' : order?.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
              {order?.status === 'accepted' ? '🛵 En camino' : order?.status === 'delivered' ? '📦 Entregado - espera confirmación' : order?.status === 'completed' ? '✅ Completado' : order?.status}
            </div>
          </div>
          {isTracking && (
            <div className="flex items-center gap-4 mt-3 pt-3 border-t border-gray-100">
              <div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${updating ? 'bg-green-500 animate-pulse' : 'bg-green-500'}`} /><span className="text-xs text-gray-500">{updating ? 'Actualizando...' : 'Ubicación en tiempo real'}</span></div>
              {distance !== null && <div className="flex items-center gap-2 text-xs text-gray-500"><FiCompass className="text-xs" /> {distance.toFixed(1)} km</div>}
              {eta !== null && eta > 0 && <div className="flex items-center gap-2 text-xs text-gray-500"><FiClock className="text-xs" /> ≈ {eta} min</div>}
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-4">
          <h3 className="font-semibold text-gray-800 mb-3 flex items-center gap-2"><FiMapPin className="text-[#FF6B35]" /> Direcciones</h3>
          <div className="space-y-3">
            <div className="flex gap-3"><div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">📍</div><div><p className="text-xs text-gray-500">Punto de recogida</p><p className="text-sm text-gray-700">{order?.pickupAddress}</p></div></div>
            <div className="flex gap-3"><div className="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0">🏠</div><div><p className="text-xs text-gray-500">Destino</p><p className="text-sm text-gray-700">{order?.deliveryAddress}</p></div></div>
          </div>
        </div>

        {isTracking && mandaditoLocation && (
          <button onClick={openInGoogleMaps} className="w-full bg-[#FF6B35] text-white py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-[#e55a2b] transition-colors"><FiMap /> Abrir en Google Maps</button>
        )}

        {!isTracking && order?.status !== 'completed' && (
          <div className="bg-gray-50 rounded-2xl p-5 text-center"><FiTarget className="text-3xl text-gray-300 mx-auto mb-2" /><p className="text-gray-500 text-sm">El seguimiento estará disponible cuando el mandadito acepte el pedido.</p></div>
        )}
      </div>
    </Background>
  );
