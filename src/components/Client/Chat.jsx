      ...
      <div className="max-w-3xl mx-auto h-screen flex flex-col">
        ...
        <div className="bg-white border-b sticky top-0 z-10 p-4 flex items-center gap-3 shadow-sm">
          ...
          <div className="flex-1">
            <h2 className="font-semibold text-gray-800">{otherUser?.name || 'Mandadito'}</h2>
        ...
        {/* Mensaje de ayuda para coordinar */}
        {order?.status !== 'completed' && (
          <div className="bg-blue-50 border-b border-blue-100 p-3 text-center">
            <p className="text-sm text-blue-700 flex items-center justify-center gap-2">
              <FiMessageCircle />
              Coordina los detalles del mandado aquí: cantidad de productos, cambios, etc.
            </p>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
          {messages.length === 0 ? (
            <div className="text-center text-gray-400 py-8">
              ...
              <p className="text-xs mt-2 text-gray-300">Ej: ¿Cuántos productos necesitas? ¿Algún cambio?</p>
            </div>
          ) : (
        ...
        {order?.status !== 'completed' && (
          <div className="bg-gray-50 border-t p-2 flex gap-2 overflow-x-auto">
            {['¿Cuántos productos?', '¿Algún cambio?', '¿Dónde te encuentras?', 'Ya casi llego', 'Confirmar entrega'].map(suggestion => (
              <button
                key={suggestion}
