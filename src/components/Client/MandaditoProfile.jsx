...
import { 
  ...
  FiShield, FiTruck, FiCamera, FiCheckCircle
} from 'react-icons/fi';
  ...
  return (
    <Background>
      <div className="max-w-4xl mx-auto py-8 px-4 pb-24 md:pb-8">
          ...
          <div className="px-6 pb-6">
            <div className="flex flex-col md:flex-row items-center md:items-end -mt-12 mb-4">
              ...
              <div className="flex gap-2 mt-4 md:mt-0">
                <button
                  onClick={handleCall}
                  className="bg-[#1E3A8A] text-white px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-[#2c4a9e] transition-colors"
                >
                  <FiPhone /> Llamar
                </button>
                <button
                ...
                {mandadito?.isAvailable && mandadito?.isVerified && (
                  <button
                    ...
                    className="bg-[#E63946] text-white px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-[#c92a2a] transition-colors"
                  >
                    <FiSend /> Asignar
                  </button>
                )}
            ...
            <div className="grid grid-cols-2 gap-3">
              {mandadito.motoPhotos.map((photo, idx) => (
                <img
                  ...
                  onError={(e) => {
                    e.target.onerror = null;
                    e.target.src = 'https://via.placeholder.com/200?text=Vehiculo';
                  }}
                />
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {mandadito?.cedulaPhoto && (
              <div className="border border-gray-200 rounded-xl p-3 text-center">
                ...
                <button 
                  onClick={() => window.open(mandadito.cedulaPhoto, '_blank')}
                  className="text-xs text-[#E63946] hover:underline mt-1"
                >
                  Ver documento
                </button>
              </div>
            ...
            {mandadito?.seguroPhoto && (
              <div className="border border-gray-200 rounded-xl p-3 text-center">
                ...
                <button 
                  onClick={() => window.open(mandadito.seguroPhoto, '_blank')}
                  className="text-xs text-[#E63946] hover:underline mt-1"
                >
                  Ver documento
                </button>
            ...
            {mandadito?.licenciaPhoto && (
              <div className="border border-gray-200 rounded-xl p-3 text-center">
                ...
                <button 
                  onClick={() => window.open(mandadito.licenciaPhoto, '_blank')}
                  className="text-xs text-[#E63946] hover:underline mt-1"
                >
                  Ver documento
                </button>
        ...
        {showOrderForm && (
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-6 border-2 border-[#E63946]/30">
            <h2 className="text-xl font-bold text-gray-800 mb-2">Asignar Mandado a {mandadito?.name}</h2>
